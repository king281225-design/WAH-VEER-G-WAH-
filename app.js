(function () {
  ...
  function initReviewComposer() {
    const starsWrap = $("#rc-stars");
    const textEl = $("#rc-text");
    const submitBtn = $("#rc-submit");
    const statusEl = $("#rc-status");
    if (!starsWrap || !textEl || !submitBtn) return;

    let rating = 0;
    const starButtons = $$(".rc-star", starsWrap);

    function paintStars(n) {
      starButtons.forEach((b) => b.classList.toggle("filled", Number(b.dataset.star) <= n));
    }
    ...
    submitBtn.addEventListener("click", async () => {
      ...
      const fullText = stars ? `${stars}\n${text}` : text;

      if (!text && !rating) {
        statusEl.textContent = "Write a few words (or pick a star rating) first — then we'll take it to Google.";
        statusEl.className = "rc-status rc-status-warn";
        return;
      }
      ...
      const dest =
        (typeof CONFIG !== "undefined" && (CONFIG.GOOGLE_WRITE_REVIEW_URL || CONFIG.GOOGLE_REVIEW_URL)) || "#";
      window.open(dest, "_blank", "noopener");

      statusEl.textContent = copied
        ? "Copied! Paste it into the Google review box that just opened, add your stars, and hit Post."
        : "Google is opening in a new tab — just copy your review above and paste it in.";
      statusEl.className = "rc-status rc-status-ok";
    });
  }
